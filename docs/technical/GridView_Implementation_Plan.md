...
3. **Staging provisioning and cutover** — the Durable Object export/binding
   ...
   **Staging provisioning — DONE (2026-09-12; see the table below).** One
   `wrangler deploy --env staging` of the Worker tree at the reviewed `master`
   commit `ea8b68a0f106f36913d386064645b79cf1c10e1b` produced the new active
   version `985115b7-abb3-4346-8845-d8ff41c80cf6` on the existing
   ...
   deployment: every edge change merged on `master` since July is now live in
   staging, and **two** Durable Object bindings and namespaces were
   ...
   (`SEASON_PUBLICATION_AUTHORITY` absent everywhere) and **admission is still
   open** (`SEASON_PUBLICATION_CUTOVER_CONTROL` absent) — no season's legacy
   publication or rollback was paused, no checkpoint was constructed or
   approved, and no seed, activation, smoke test, provider request or endpoint
   call occurred. Production was not deployed, changed, or contacted, and
   still does not exist as a Worker on the account. What remains, in order,
