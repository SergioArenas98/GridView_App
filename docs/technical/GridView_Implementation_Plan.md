...
| Proof | Edge: `participation-integrity.test.ts`, `season-driver-summary.test.ts`, the two `test/scripts` suites and the reworked span and identity suites, on an authored split season (Lawson at racing-bulls to round 11 and red-bull from round 12, Tsunoda from round 12, Hadjar to round 11). The generator-derived `drivers/season-drivers-split.json` joins the contract fixtures. Client: `split_participation_test.dart`, `participation_span_wording_test.dart` and widget tests. The `driver_detail_partial` golden changed only because the longer copy wraps. Five targeted negative controls each made tests fail and were restored byte for byte. |
