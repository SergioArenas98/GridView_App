...
| Event registry and `event` mapping support | **Not implemented.** Required shape: `event` in the mapping entity union, a typed injective composite locator key, events in `CanonicalRegistries`, JSON Schema and `validate:content` coverage (A5). |
| Event mapping dataset | **Not created.** No complete locator is recorded in the repository; seeding one needs separately authorized evidence. |
...
- Implement the curated **event registry** and the `event` mapping entity with
  its complete-tuple Jolpica locator, schemas and `validate:content` coverage,
  then curate event mappings from separately authorized evidence (§14.0.12,
  [ADR 0022 amendment](../adr/0022-curated-provider-identifier-mappings.md#amendment-2026-09-16-grand-prix-event-identity)
  A1-A5). **Prerequisite of every Jolpica resource that produces a `GrandPrix`
  or `Session`.**
