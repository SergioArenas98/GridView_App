...
| **Event dataset** | **Not created.** The registry is empty and no complete locator is recorded anywhere (A4). Tracked as **G-l**. |
...
separately authorized evidence - **not adapter registration.**
...
- ~~Implement the curated **event registry** and the `event` mapping entity
  ...
  evidence.** The registry is committed empty and no complete locator is
  recorded, so this remains a **prerequisite of every Jolpica resource that
  produces a `GrandPrix` or `Session`.**
