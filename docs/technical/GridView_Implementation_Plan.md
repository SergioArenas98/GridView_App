...
### 14.0.14 Phase 9B 2026 event dataset - curated, dormant, circuits outstanding
...
| **Circuit coverage** | **Open.** 22 of the 23 observed Jolpica circuit identifiers have no curated circuit mapping (only `albert_park`; `hungaroring` is acknowledged unmapped), and an event mapping never implies a circuit, so a functioning calendar adapter remains blocked. |
...
| **G-l** | **Open.** Only its 2026 event-identity sub-gap is closed. |
| **G1, G5, G9** | **Open.** |
| Provider modes | **Unchanged.** `PROVIDER_MODE` admits exactly `mock` and `none`; staging is `mock`, production is `none`. |
...
- Curate **circuit coverage** for the 2026 calendar: 22 of the 23 observed
  Jolpica circuit identifiers have no curated circuit mapping, which blocks
  every Jolpica resource that produces a `GrandPrix` or `Session`.
