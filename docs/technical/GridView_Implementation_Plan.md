...
| Review corrections (2026-09-10) | **Two reproduced PR #19 findings corrected, 55 regression tests added, 2581 total.** (1) An identical seed retry recomputed its high-water mark from a later clock and reported `conflicting-cutover-seed`; the service now retrieves the seed committed under the checkpoint's fingerprint first — through a read-only, request-bound `recover-cutover-seed` sequencer command with a named, cross-checked decoder — and re-presents it unchanged, so a retry reuses the committed seed and floor without re-reading legacy artifacts, while a different checkpoint still conflicts and an irreconcilable committed seed fails closed as `committed-seed-incoherent`. (2) Provenance resolution now shares the bounded retry budget, retrying only an unreadable sidecar; malformed, absent-required and invalid legacy provenance still fail at once. No fingerprint, authority or configuration change, and no deployment, provisioning, seed or activation. |
...
3. **Staging provisioning and cutover** — the Durable Object export/binding
   ...
   **Staging cutover preparation — DONE (2026-09-10; see the table below).**
   ...
   activated.** Everything else in this item remains separately authorized:

   1. staging provisioning and deployment with admission closed;
   2. operator checkpoint and seed;
   3. separate activation confirmation and mutation resumption;
   4. smoke and latency review;
