...
Status: **Phase 8C-2 complete — external Firebase verification confirmed
(2026-08-16).** A controlled fatal, a controlled non-fatal and the `gv_sync_run`
custom trace were generated by a production-flavor **debug** build on a dedicated
clean emulator, accepted by Firebase's ingestion endpoints, and subsequently
**observed in Firebase Console**. See §9 for exactly what was and was not
confirmed.
...
The database is opened **once per process**, lazily, on first use — which happens
during the first frame, inside the activation window, while
`DeferredPerformanceTracer` is still delegating to the no-op. So on an ordinary
launch the open is not recorded, and across the Phase 8C-2 runs `gv_database_open`
was never produced. The **startup** `gv_sync_run` is lost the same way; later
foreground, resumed and manual runs fall outside the window and are recorded
normally, which is what Phase 8C-2 confirmed in Console.
...
instance opened after adoption — a later `ProviderScope`, as a test or a future
multi-scope shell would create — still produces the trace. The instrumentation is
therefore kept as a best-effort signal rather than removed.
...
- Phase 8C-2 **did** send exactly one controlled fatal and one controlled
  non-fatal, and generated the `gv_sync_run` trace, from a production debug build.
  See §9. Nothing else was sent.

## 9. Verification status

**Phase 8C-2 external verification is complete (2026-08-16).** One controlled
fatal, one controlled non-fatal and the `gv_sync_run` custom trace were generated
by a **production-flavor debug** build (`APP_ENV=production`) running on a
dedicated, clean emulator with no prior GridView installation, and were then
...
- **No release build, no APK/AAB publication, no Play Console access.** The
  verified artifact was a production **debug** build; observability behaviour is
  identical because eligibility keys off `APP_ENV`, not the build type.
- **No mapping-file or native-symbol upload.** Authorization was absent
  throughout and the upload task was not registered (§8).
- The two controlled events **remain stored in the production Firebase project**.
  They were deliberately not deleted, so this record stays checkable.
- The temporary verification harness that drove the two signals was **fully
  removed** from the working tree. It was compile-time gated behind a define
  absent from every ordinary build, added no UI, route or production-reachable
  trigger, and **must not be reintroduced**. There is still no force-crash
  control, hidden crash route or automatic test exception anywhere in the app.
...
availability, nor of behaviour under every device or network condition.
...
- **No NDK crash capture** and the two Play blockers — an unset privacy-policy URL
  and the outstanding Data Safety declaration — remain open. See
