...
- **`audi` continues the existing `sauber` identity.** Jolpica now returns
  ...
  provider's ID change is not a GridView lineage ruling. Under the Domain
  Model §6.8 rebranding rule, the stable, immutable ID `sauber` is kept, and
  its display name becomes **`Audi`**, the team's current branding. **No
  separate `audi` identity was created.** The other properties of the
  `sauber` registry row are unchanged.
...
| `audi` | `sauber` | `Audi` | Lineage: existing `sauber` continued, display name changed to Audi |
