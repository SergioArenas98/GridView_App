...
- **Circuit coverage is complete for this calendar.** The 23 `circuitId`s are
  ...
  its own record. **Circuit coverage no longer blocks a calendar adapter; no
  such adapter exists.**
...
with a deterministic `retryAt`. **No adapter exists, so nothing is paced yet
and no provider request is sent.** Acting on `retryAt` is G5 event-aware
scheduling, which remains open.
