...
- Phase: 9A (provider evaluation and licensing basis), extended by the Phase 9B-0 entry decisions, the Phase 9B-1 accounting/quota implementation, the Phase 9B-2 outbound hardening and rate limiter, and the Phase 9B-3 identifier mapping registry
- Related documents:
...
- Research access date: **2026-08-19** (no new external research was performed for versions 0.4 or 0.5; no inquiry has been sent to any provider). One further authorised calendar observation is dated **2026-09-19** (§8.8)
...
| 0.4 | 2026-08-21 | **Phase 9B-0 entry decisions recorded** ([ADR 0020](../adr/0020-provider-source-observation-and-reconciliation.md)). E5a is decided: the snapshot-level `snapshotObservedAt`, bound to `snapshotRevision`, is adopted as the value published under the contract-required `sourceUpdatedAt`, while the resource-level `sourceObservedAt` stays internal reconciliation state and is never published (§10.7.1, ADR 0020 D1.9, D1.12), and the residual reconciled-ordering risk is accepted with monitoring (§10.9.1). E5b is satisfied: I1-I5 are binding, and the settling state machine, cadence bounds and slow post-settlement sweep are now **specified** in §10.4.1 rather than deferred, which closes gap G-j and bounds the volume model (§11.3.1). E6 is specified and binding, with the OpenF1 bound-or-skip rule and the explicit absence of exceptions. **No implementation, no adapter, no live request and no licensing change.** |
| 0.5 | 2026-08-23 | **Phase 9B implementation started with Phase 9B-1** (source-aware provider accounting and quota foundation). **G6** and **G-h** are closed: provider request accounting is typed at compile time, per canonical source and per job category, counting successful, failed and rate-limited attempts. **G10** and **G-k** are closed: `QuotaState` is per source with an extensible window collection matching what each source publishes — OpenF1 per-second and per-minute, Jolpica per-second and per-hour, **no daily bucket for either** — persisted under `quota:provider:<sourceId>`, with the §16.1 warning thresholds implemented. Stale provenance wording in the 0.4 entry, §14.4 G-i and §15.2 E5a is corrected to say that the **snapshot-level** `snapshotObservedAt` is what reaches the wire. **G4, G7 and every other gap remain open. No adapter, no outbound request, no rate limiter, no coordinator, no deployment, no provider contact, and no licensing change.** `PROVIDER_MODE` still admits exactly `mock` and `none`, production remains `"none"`, and the OpenF1 path stays fail-closed. |
| 0.6 | 2026-08-23 | **Phase 9B-2: G7 closed** ([ADR 0021](../adr/0021-hardened-provider-boundary-and-durable-object-rate-limiter.md)). One hardened outbound boundary supplies every Backend Scheme §23.3 control - pinned origins and path prefixes, GET only, no followed redirects, a 10-second timeout, JSON-only content types, a 2 MiB streamed body cap, no automatic retry, no forwarded caller header or credential, and Jolpica's identifying `User-Agent` as a reviewed constant. The per-provider rate limiter is a **Durable Object with one identity per real source**, performing exact sliding-window reservations across every published window atomically; isolate state, KV and Cloudflare's native Rate Limiting binding were rejected as the authority and the reasons are recorded. G-f is closed with it. **No adapter, no provider request, no provisioned or deployed Cloudflare resource, no licensing change.** `PROVIDER_MODE` still admits exactly `mock` and `none`, production remains `"none"`, and OpenF1 stays fail-closed. G4, G5, G8 and G9 remain open. |
| 0.8 | 2026-08-26 | **Phase 9B-4: G4 and G-c closed as a coordination mechanism** ([ADR 0023](../adr/0023-multi-source-provider-coordination.md)). The single-call `fetchSeasonSource(season, jobs)` whole-season assumption is superseded by a typed multi-source coordination seam: **independent per-source resource ports** that never import, call, inspect or know about one another; a closed discriminated union of per-resource requests carrying source, canonical resource identity and season/round/session scope, with the job category **derived** from the resource kind so attribution is exact and `home-rebuild` can never become a provider request; source **role and capability owned above the adapters** (Jolpica `reconciled` over every resource, OpenF1 `provisional` over the documented post-session result and championship resources only, with no telemetry, live-timing, media, baseline-metadata or health-check exception); a closed outcome taxonomy in which **not-attempted is structurally unrepresentable as an attempt**; deterministic selection by declared role alone, so a provisional payload never overwrites a reconciled one and neither arrival order, plan order, payload size nor string similarity can influence it; exact **once-per-transport-request** accounting, including one request serving several derived resources; fail-closed rejection of a duplicate plan with nothing attempted; mapping failures contained to the affected source contribution without duplicating the Phase 9B-3 signal; bounded cancellation and bounded concurrency; and a guarded bridge that calls the unchanged publisher **at most once** for a complete run and withholds every partial one, preserving atomic publication and last-known-good. **The seam is dormant: no adapter consumes it, no port is registered in production wiring, no provider was contacted, no request was made or made possible, nothing was deployed, and no licensing conclusion changes.** `PROVIDER_MODE` still admits exactly `mock` and `none`, production remains `"none"`, and OpenF1 stays fail-closed - the coordinator consumes an already-decided session-end bound and **none is recorded**. G5, G9 and G-l remain open. |
| 0.7 | 2026-08-25 | **Phase 9B-3: G8 and G-e closed** ([ADR 0022](../adr/0022-curated-provider-identifier-mappings.md)). A curated, version-controlled, season-qualified provider-identifier mapping registry, keyed on season, source, entity kind, exact provider field and exact provider value together, matched by **exact typed equality** - no case folding, trimming, slug minting, transliteration, substring or fuzzy matching, and integer `1` is never string `"1"`. Season-qualification is forced for OpenF1, whose `driver_number` is reassigned between seasons (§8.7 M2). Validation is structural (JSON Schema 2020-12) and semantic (composite-key uniqueness, target existence, evidence coverage), both inside `npm run validate:content`; one invalid record blocks the whole resolver and no partial index is ever exposed. Unknown entities fail closed with a bounded operational signal. Only identifiers **already recorded in §8** are seeded: `Cadillac` and `Racing Bulls` remain unmapped because no canonical GridView constructor identity exists for them, and no OpenF1 `circuit_key` value is recorded anywhere - dataset coverage is therefore incomplete and is tracked as the new gap **G-l**. **The registry is dormant: no adapter consumes it, no provider was contacted, no request was made or made possible, nothing was deployed, and no licensing conclusion changes.** `PROVIDER_MODE` still admits exactly `mock` and `none`, production remains `"none"`, and OpenF1 stays fail-closed. G4, G5 and G9 remain open. |
| 0.9 | 2026-09-02 | **Phase 9B-5: deep normalized-contract validation closed as a mechanism** ([ADR 0024](../adr/0024-deep-normalized-contract-validation.md)), amending [ADR 0023](../adr/0023-multi-source-provider-coordination.md) D14. An authoritative per-field validator covers every entity the seven `CoordinatedPayload` variants carry, with property presence taken from `contract/types.ts` and value rules from the OpenAPI schema, and **only the bounds the contract states** - no sign or range rule is invented for wins, podiums, laps, lengths, corner counts, coordinates, aspect ratios, durations, gaps or points. Unknown own properties are **refused**, because the snapshot generator carries a normalized entity into the detail, results, standings, home and bootstrap documents verbatim; that is the producing direction and does not disturb the tolerant-consumer posture, and both consumer-tolerance fixtures are asserted to be refused by it. `unknown` remains a valid enum member; a raw upstream token is refused. Validation runs at the **coordination boundary** on the same detached snapshot that is later selected, assembled and published, and a failure is the **existing** `invalid-payload` attempted-failure contribution - attempted, counted exactly once, never selected, never assembled, never published, with a healthy fallback still able to carry the resource - carrying bounded structural paths and closed codes that reach neither a contribution nor a log line. The deferred **F3**, **F4** and **F5** findings are closed as three independent relations (`driver-entry-span`, `event-identity`, `constructor-entry-identity`) and recorded in the repository rather than only in PR #12's discussion. **Dormant: no adapter consumes any of it, no provider was contacted, no request was made or made possible, nothing was deployed, and no licensing conclusion changes.** `PROVIDER_MODE` still admits exactly `mock` and `none`, production remains `"none"`, and OpenF1 stays fail-closed with no recorded bound. G1, G3, G5, G9 and G-l remain open, both adapters remain unimplemented, and the attribution and ShareAlike surfaces remain outstanding. |
| 1.0 | 2026-09-03 | **Phase 9B-6 (PR 1): `snapshotRevision` implemented as a mechanism; the observation clock is blocked.** The binding canonical revision input of [ADR 0020](../adr/0020-provider-source-observation-and-reconciliation.md) D1.7 now exists and is tested: a schema-aware canonical representation of the normalized public `data` payload plus its `schemaVersion`, **constructed** from a per-snapshot-key declaration rather than filtered out of a serialized envelope, so every exclusion holds by construction - including `HomeData.freshness`, the one place where excluded metadata (`generatedAt`, `sourceUpdatedAt`, `staleAfter`, the server-stale flag) lives *inside* `data`. Deterministic UTF-8 byte key ordering with a dedicated comparator (JavaScript's default UTF-16 unit order is not the documented rule), domain order preserved for ordered arrays, exactly two arrays declared unordered with their stable GridView identity, absent/null collapsed only for the properties the contract declares with `?`, RFC 3339 canonicalized to UTC **without truncating fractional precision** and without `Date` (which would roll a leap second), one canonical numeric spelling, and SHA-256 over a length-framed canonical text prefixed `gv-canon/1`, rendered `sha256:<64 hex>`. **`snapshotObservedAt` is not implemented and D1.10 is blocked**: the strictly-monotonic assignment must be computed pre-commit from the pair the active pointer names, and two publications for one season can both reach `SnapshotPublisher` (the staging cron and the protected `/internal/admin/sync/full`, which forces every job and always publishes) without observing each other, so two changed revisions can receive equal or decreasing timestamps. Workers KV offers no compare-and-set and no cross-isolate lock ([ADR 0007](../adr/0007-versioned-kv-publication-active-pointer.md), [ADR 0010](../adr/0010-workers-kv-consistency-limitation.md)); closing it needs a serialization mechanism that is an infrastructure decision this pass was not authorized to take. **`meta.sourceUpdatedAt` is unchanged** - still provider-supplied, staging still publishing the mock provider's constant - and the module has **no production caller**. **G-i therefore stays open in both halves.** **G3 gains an explicit prerequisite: production operational runbooks.** Dormant throughout: no adapter, no provider contacted, no request made or made possible, nothing deployed, no licensing conclusion changed. `PROVIDER_MODE` still admits exactly `mock` and `none`, production remains `"none"`, and OpenF1 stays fail-closed with no recorded bound. G1, G3, G5, G9 and G-l remain open. |
...
A small number of **unauthenticated public `GET` requests** — roughly 25 — was
made on 2026-08-19 under the authorisation recorded in §8.1, solely to validate
the data mapping. **One further unauthenticated `GET`**, separately authorised,
was made on 2026-09-19 to record the 2026 calendar's event locators (§8.8). No
credential, cookie or token was used; no paid or live endpoint was touched; no
image was downloaded; no response was retained inside the repository. None of
these requests was made by GridView's software: the Worker, its hardened
provider client and its rate limiter have made no provider request.
...
One further request, separately authorised on 2026-09-19 under the same
restrictions, is recorded in §8.8.
...
> **Note 2026-09-23 (v1.8).** The table above is the 2026-08-19 observation and
> is left as recorded. Under
> [ADR 0026](../adr/0026-season-participation-semantics-and-derivation.md) the
> 31 `/2026/drivers/` rows are treated as the season's **identity universe**,
> not as a count of race participants. Whether any extra row ever raced is
> **unverified**: this same pass recorded 22 `driverstandings` rows at round 11,
> and no response was preserved. The "strong evidence of substantial mid-season
> driver churn" reading is therefore an interpretation awaiting evidence.
> Participation spans come only from selected race classifications.
...
### 8.8 2026 calendar observation and the curated event dataset (2026-09-19)

One further public `GET` was **separately and explicitly authorised on
2026-09-19**, solely to collect evidence for the curated Grand Prix event
dataset. It is recorded here because [ADR 0022](../adr/0022-curated-provider-identifier-mappings.md#a4---evidence-and-review-are-at-least-as-strict-as-existing-mappings)
amendment A4 requires every event mapping's evidence to point inside this
repository. It was a one-off request from a developer machine, **not** a request
by GridView's Worker, its hardened provider HTTP client or its rate limiter:
GridView's application code has still made no provider request. No other
request, to Jolpica or to OpenF1, accompanied it.

| Field | Value |
|---|---|
| Provider | Jolpica F1 |
| Endpoint | `https://api.jolpi.ca/ergast/f1/2026/races/?limit=100` |
| Method | One unauthenticated `GET`. No credential, cookie or token; no redirect followed; no retry |
| Request window (UTC) | Started `2026-09-19T19:38:14.947Z`, completed `2026-09-19T19:38:15.183Z`; response `Date` `2026-09-19T19:38:15Z` |
| Status | HTTP 200, `application/json` |
| Count | `limit` 100, `offset` 0, `total` 23; 23 race objects returned, so the declared and returned counts agree |
| Body | 14 295 bytes; SHA-256 `87dd8cad5d33eb67f97aa46de7d024715135b9429707de99f8966c498c8e0bed` over the exact bytes received |
| Freshness | Served from Jolpica's CDN cache (`cf-cache-status: HIT`, `Age: 135`), so it reflects Jolpica's state when that response was generated |
| Structural checks | Strict UTF-8 JSON; every race carries `season` "2026"; rounds unique, contiguous 1-23 and ascending; no `raceName` or `circuitId` has edge whitespace; every complete tuple unique; no `circuitId` hosts two events |
| Raw capture | Held by the operator **outside the repository and not committed**, like the §8.1 responses. This section is the repository-owned record; the hash above lets a reviewer holding the capture confirm it is the one summarised here |
...
**Official corroboration.** The official Formula 1 2026 calendar,
`https://www.formula1.com/en/racing/2026`, **accessed 2026-09-19**, lists the
same 23 rounds in the same order. It is used only to corroborate each event's
identity. It is not a licensed data source, nothing from it is committed beyond
the location labels in the table below, and no Formula 1 name or mark is
licensed to GridView (§0.2).

**Curator decisions, approved 2026-09-19.** Every `eventSlug` below is an
explicit curator decision recorded in the reviewed change that added it, as
ADR 0022 amendment A1 requires. None was derived by software from a `raceName`:

- **Round 7** is the distinct Barcelona-Catalunya event and uses
  `barcelona-grand-prix`.
- **Round 14** is the Madrid event, which retains the official Spanish Grand
  Prix identity and uses `spanish-grand-prix`.
- **Round 16** retains the Bahrain Grand Prix identity despite its 2026
  relocation to Sepang. The circuit belongs to the season's provider locator,
  not to the identity.
- **Round 20** uses the official contemporary event identity
  `sao-paulo-grand-prix`, although Jolpica exposes `Brazilian Grand Prix`.
- **Eight slugs that already appeared in mocks, contract fixtures or tests** -
  `australian-grand-prix`, `japanese-grand-prix`, `monaco-grand-prix`,
  `british-grand-prix`, `belgian-grand-prix`, `hungarian-grand-prix`,
  `dutch-grand-prix` and `italian-grand-prix` - are adopted explicitly here.
  Their earlier appearance was not the approval, and the mock rounds they
  carried were never evidence.
- `french-grand-prix`, `mystery-grand-prix` and `test-grand-prix` are synthetic
  test identities and are **not** canonical.

Each mapping's locator is the exact observed tuple; its season is the season of
the file it lives in, never a field of the record.

| Round | Exact `raceName` | Exact `circuitId` | Curated `eventSlug` | Official calendar, 2026-09-19 |
|---|---|---|---|---|
| 1 | `Australian Grand Prix` | `albert_park` | `australian-grand-prix` | Round 1, Australia |
| 2 | `Chinese Grand Prix` | `shanghai` | `chinese-grand-prix` | Round 2, China |
| 3 | `Japanese Grand Prix` | `suzuka` | `japanese-grand-prix` | Round 3, Japan |
| 4 | `Miami Grand Prix` | `miami` | `miami-grand-prix` | Round 4, Miami |
| 5 | `Canadian Grand Prix` | `villeneuve` | `canadian-grand-prix` | Round 5, Canada |
| 6 | `Monaco Grand Prix` | `monaco` | `monaco-grand-prix` | Round 6, Monaco |
| 7 | `Barcelona Grand Prix` | `catalunya` | `barcelona-grand-prix` | Round 7, Barcelona-Catalunya |
| 8 | `Austrian Grand Prix` | `red_bull_ring` | `austrian-grand-prix` | Round 8, Austria |
| 9 | `British Grand Prix` | `silverstone` | `british-grand-prix` | Round 9, Great Britain |
| 10 | `Belgian Grand Prix` | `spa` | `belgian-grand-prix` | Round 10, Belgium |
| 11 | `Hungarian Grand Prix` | `hungaroring` | `hungarian-grand-prix` | Round 11, Hungary |
| 12 | `Dutch Grand Prix` | `zandvoort` | `dutch-grand-prix` | Round 12, Netherlands |
| 13 | `Italian Grand Prix` | `monza` | `italian-grand-prix` | Round 13, Italy |
| 14 | `Spanish Grand Prix` | `madring` | `spanish-grand-prix` | Round 14, Spain |
| 15 | `Azerbaijan Grand Prix` | `baku` | `azerbaijan-grand-prix` | Round 15, Azerbaijan |
| 16 | `Bahrain Grand Prix in Malaysia` | `sepang` | `bahrain-grand-prix` | Round 16, Bahrain (held in Malaysia) |
| 17 | `Singapore Grand Prix` | `marina_bay` | `singapore-grand-prix` | Round 17, Singapore |
| 18 | `United States Grand Prix` | `americas` | `united-states-grand-prix` | Round 18, United States |
| 19 | `Mexico City Grand Prix` | `rodriguez` | `mexico-city-grand-prix` | Round 19, Mexico |
| 20 | `Brazilian Grand Prix` | `interlagos` | `sao-paulo-grand-prix` | Round 20, Brazil (São Paulo) |
| 21 | `Las Vegas Grand Prix` | `vegas` | `las-vegas-grand-prix` | Round 21, Las Vegas |
| 22 | `Qatar Grand Prix` | `losail` | `qatar-grand-prix` | Round 22, Qatar |
| 23 | `Abu Dhabi Grand Prix` | `yas_marina` | `abu-dhabi-grand-prix` | Round 23, Abu Dhabi |

All 23 identities are in `content/registries/events.development.json`, and all
23 locators are mapped in `content/seasons/2026/provider-mappings.development.json`
...
#### 8.8.1 Circuit evidence for the 23 season-2026 circuits

Each race in the same response carries a `Circuit` object, so the single
response recorded at the top of §8.8 supplies all 23 observed `circuitId`s.
Every one of them now has a canonical GridView circuit identity, a reviewed
mapping and an evidence-corpus entry, reached in two recorded passes: **five
mapping-only decisions on 2026-09-19**, where the canonical identity already
existed, and **seventeen curated identities approved on 2026-09-20**, where it
did not. An event mapping never implies a circuit ([ADR 0022](../adr/0022-curated-provider-identifier-mappings.md)
...
##### The five mapped on 2026-09-19 (mapping decisions only)

Five of the 23 observed `circuitId`s name a venue for which a canonical GridView
circuit identity **already existed** in `content/registries/circuits.mock.json`,
so each pair below was a **mapping decision only**: no circuit identity was
created, renamed or removed for any of them.
...
##### The seventeen curated on 2026-09-20 (new canonical identities)

The remaining 17 observed `circuitId`s had no canonical GridView circuit
identity, so a curator created one for each. Every canonical ID and display name
below is **curator-authored**: no provider name was adopted as an identity, no
alias was created and no provider value was normalised into an ID.

The canonical ID is **immutable**. The display name carries the venue's current
public branding and **may be updated later** if that branding changes. Three
decisions are recorded explicitly:

- **A commercially named venue takes a geographic ID.** `red_bull_ring` and
  `madring` name venues whose public branding is commercial and can change, so
  the curated IDs are `spielberg` and `madrid` — the geography, which does not
  change when a sponsor does — while the display names `Red Bull Ring` and
  `Madring` carry the branding.
- **An official name wins over a colloquial one.** `interlagos` and `losail` are
  the provider's colloquial and alternative-romanisation forms; the curated
  identities are the official `jose-carlos-pace` and `lusail`.
- **Generic circuit words are stripped from the ID** and kept in the display
  name. Where that leaves the curated ID equal to the provider value —
  `hungaroring` and `sepang`, approved as canonical IDs in their own right, and
  `shanghai`, `miami`, `zandvoort` and `baku` — the agreement is a curator
  decision, never a matching rule.

| Round | Exact `circuitId` | Canonical GridView circuit | Canonical display name |
|---|---|---|---|
...
**Each new registry row carries only `id` and `name`.** No locality, country,
coordinates, length, corner count, direction, first-Grand-Prix year or lap
record was committed for any of them. Those are descriptive facts GridView does
not own for these venues, and the observed `Circuit` object is not a licence to
publish them; the identity is all this dataset needs and all it records. The
curated circuit registry therefore holds **23 identities**, six of them
pre-existing rows with illustrative development facts and seventeen of them
identity-only.

**What this does and does not cover.** Curated circuit coverage of the observed
2026 calendar is now **23 of 23**, and **no observed 2026 `circuitId` is
acknowledged as unmapped**. The season-2026 dataset holds **53 exact mappings**
and **57 approved evidence identities**, with **four acknowledgements**
remaining — `antonelli`, OpenF1 `driver_number` `12`, `Cadillac` and
`Racing Bulls`, none of them a circuit.

**Circuit coverage no longer blocks the calendar adapter, and no adapter
exists.** The two statements are independent and both are true: the dataset gap
that blocked a Jolpica calendar adapter is closed, and **no Jolpica adapter has
been written**. A complete mapping dataset is not an adapter, a runtime path or
a production capability. `PROVIDER_MODE` still admits exactly `mock` and `none`,
production remains `"none"`, **no live provider mode has been enabled**, and
**G1 remains open**. The **circuit portion of G-l is complete**; **G-l itself
remains open** because the four non-circuit identities above are still
acknowledged as unmapped and no OpenF1 `circuit_key` value is recorded anywhere.

This coverage is a **point-in-time observation** of the calendar as it stood on
2026-09-19. A later calendar change fails closed until another reviewed mapping
update.
...
- **It is a point-in-time observation from 2026-09-19.** Rounds 15-23 were
  still in the future. A later calendar change, rename, round shift or venue
  change produces a tuple no record matches, so the calendar **fails closed**
  until another reviewed mapping update lands on new, separately authorised
  evidence (ADR 0022 amendment A3; Mapping Guide §15.3).
- **`round` arrives as a JSON string** (`"1"`); the curated locator stores an
  ...
  implemented.
- **Circuit coverage is complete for this calendar.** The 23 `circuitId`s are
  locator components in the event table above, and every one of them now also
  has a curated circuit identity and mapping in its own right - `albert_park`
  from §8.4, five approved in §8.8.1 on 2026-09-19 and 17 approved there on
  2026-09-20. An event mapping still never implies a circuit (A3), so each is
  its own record. **Circuit coverage no longer blocks a calendar adapter.**
  **Superseded 2026-09-21** (v1.6, Implementation Plan §14.0.16): a **dormant,
  fixture-tested `season-calendar` port** now consumes this coverage when
  exercised directly by tests. It is outside the runtime composition and the
  Worker entry point's import closure, is not selectable through
  `PROVIDER_MODE`, and is invoked by no production coordinator or event-aware
...
> row changed. **Note 2026-09-23 (v1.10):** the 2026 driver dataset (§8.10)
> brought the totals to **93** exact mappings, **96** approved evidence
> identities and **three** acknowledgements; no circuit row changed.
...
> **Note 2026-09-23 (v1.10).** The totals and the driver-coverage statement
> above are those of the constructor dataset and are left as recorded. The
> 2026 driver dataset (§8.10) then curated and mapped all 32 observed Jolpica
> `driverId`s, so the season-2026 dataset holds **93** exact mappings, **96**
> approved evidence identities and **three** acknowledgements, all OpenF1.
> No constructor row, mapping or evidence record changed.

### 8.10 2026 driver identity observation and the curated driver dataset (2026-09-23)

The driver half of the two season-2026 Jolpica participant-list `GET`s
**separately and explicitly authorised on 2026-09-23** (§8.9) is recorded here,
solely as identity evidence under
[ADR 0026](../adr/0026-season-participation-semantics-and-derivation.md). The
request came from a developer machine. It was **not** made by GridView's
Worker, its hardened provider HTTP client or its rate limiter.

| Field | Value |
|---|---|
| Provider | Jolpica F1 |
| Endpoint | `https://api.jolpi.ca/ergast/f1/2026/drivers/?limit=100` |
| Method | One unauthenticated `GET`, with no credential, cookie or token, no redirect followed and no retry |
| Request window (UTC) | Started `2026-09-23T18:33:12.832Z`, completed `2026-09-23T18:33:12.964Z`; response `Date` `2026-09-23T18:33:12Z` |
| Status | HTTP 200, `application/json` |
| Count | `limit` 100, `offset` 0, `total` 32; 32 driver rows returned, so the declared and returned counts agree |
| Body | 5 598 bytes; SHA-256 `2af29a2ae8fe8d3c1f2774d708fa9f8594ff27be69e0b2ce70b1d0513a4f0743` over the exact bytes received |
| Cache headers | `cf-cache-status: MISS` together with `Age: 202`; recorded as observed and not interpreted |
| Structural checks | Strict UTF-8 JSON with no BOM; `DriverTable.season` is `"2026"`; 32 unique `driverId`s, each a non-empty `[a-z0-9_]+` string. Nine rows carry only `driverId`, `givenName` and `familyName` |
| Raw capture | Held by the operator **outside the repository and not committed**, like the §8.1, §8.8 and §8.9 responses. This section is the repository-owned record; the hash above lets a reviewer holding the capture confirm it is the one summarised here |
| Curator decision pack | A private pack built only from this capture, SHA-256 `faa23a5f9acad648489515a82e8d57f15165762c79d7732c3613a7694764c47b`, also held outside the repository and not committed. The curator approved its recommended driver payload on 2026-09-23; it is the table below |

**Licence and attribution** ([ADR 0019](../adr/0019-formula-one-provider-legal-gate.md),
§7.2, §7.6.2, §7.6.3). The `driverId` values below are **Jolpica F1** data
(`https://github.com/jolpica/jolpica-f1`), licensed under **CC BY-NC-SA 4.0**
(`https://creativecommons.org/licenses/by-nc-sa/4.0/`). GridView records each
exact `driverId` and pairs it with a curated GridView driver; that pairing is
GridView's modification. ShareAlike applies to this recorded data as §7.6.3
describes. The provider values are internal ([ADR 0022](../adr/0022-curated-provider-identifier-mappings.md)
D10) and are published nowhere.

**Curator decisions, approved 2026-09-23.** Every canonical ID and display name
below is **curator-authored**. The given and family name in each provider row
supports it, but none was copied from the provider by pass-through or by rule.

- **Canonical IDs use the complete recorded given and family name**, slugged
  once by the curator under Domain Model §4.1: lowercase ASCII, diacritics
  transliterated, punctuation dropped, spaces and underscores replaced by one
  hyphen. A surname-only provider `driverId` (`albon`, `hamilton`) is never a
  canonical ID, and no adapter ever applies this rule. A compound given name
  keeps every token: `antonelli` maps to **`andrea-kimi-antonelli`**.
- **IDs are ASCII; display names keep their diacritics**, stored NFC. A
  curator never adds a diacritic that was not observed and never strips one
  that was: `nico-hulkenberg` is displayed `Nico Hülkenberg` and
  `sergio-perez` is displayed `Sergio Pérez`.
- **The nine name-only rows are identities.** `paul_aron`, `dino_beganovic`,
  `luke_browning`, `jak_crawford`, `leonardo_fornaroli`, `colton_herta`,
  `ryo_hirakawa`, `ayumu_iwasa` and `frederik_vesti` carry nothing beyond a
  name. ADR 0026 D2 requires every `/drivers/` row to resolve, so each gets an
  identity-only row. Nothing is inferred about team, role, activity or rounds;
  their presence in the list is recorded only as "listed".
- **No provider descriptive field was imported.** The **25 new identities**
  carry **only `id` and `fullName`**. The provider's separate given and family
  name fields, code, number, nationality, date of birth and URL are evidence
  only: they are licensed provider facts, and ADR 0026 D2 forbids provider
  display fields becoming canonical content by pass-through. The
  content-loading seam supplies the explicit `null` the normalized contract
  needs for every absent optional fact.
- **`max-verstappen` and `lando-norris` no longer carry `permanentNumber`.**
  The captured field behaves as the driver's season car number - it gives
  Norris `1` (§8.7 M2) - so it cannot establish the career permanent number
  that the Domain Model §6.2 field means. The illustrative mock values are
  unsourced, so both are removed and no other number replaces them. The season
  car number belongs in `DriverSeasonEntry.raceNumber`. No other property of
  any pre-existing row changed.
- **Six provider values** - `colapinto`, `hamilton`, `leclerc`,
  `max_verstappen`, `piastri` and `russell` - map to identities that already
  existed. These are mapping decisions only.
- **`norris`** was already mapped (§8.4) and is unchanged.
- **`jack-doohan`** stays a canonical identity. No captured row maps to it,
  and no Jolpica mapping was invented for it.
- **OpenF1 `driver_number` `12` stays acknowledged and unmapped.** It refers to
  the same competitor as `antonelli` (§8.5). A canonical identity now exists,
  so its reason changes to `no-approved-provider-mapping`. Mapping it needs a
  separate curator decision.

| Exact `driverId` | Canonical GridView driver | Canonical display name | Decision |
|---|---|---|---|
| `albon` | `alexander-albon` | `Alexander Albon` | New identity-only row |
| `alonso` | `fernando-alonso` | `Fernando Alonso` | New identity-only row |
| `antonelli` | `andrea-kimi-antonelli` | `Andrea Kimi Antonelli` | New identity-only row; the Jolpica acknowledgement is removed |
| `arvid_lindblad` | `arvid-lindblad` | `Arvid Lindblad` | New identity-only row |
| `ayumu_iwasa` | `ayumu-iwasa` | `Ayumu Iwasa` | New identity-only row from a name-only provider row |
| `bearman` | `oliver-bearman` | `Oliver Bearman` | New identity-only row |
| `bortoleto` | `gabriel-bortoleto` | `Gabriel Bortoleto` | New identity-only row |
| `bottas` | `valtteri-bottas` | `Valtteri Bottas` | New identity-only row |
| `colapinto` | `franco-colapinto` | `Franco Colapinto` | Mapping only; identity already existed |
| `colton_herta` | `colton-herta` | `Colton Herta` | New identity-only row from a name-only provider row |
| `dino_beganovic` | `dino-beganovic` | `Dino Beganovic` | New identity-only row from a name-only provider row |
| `frederik_vesti` | `frederik-vesti` | `Frederik Vesti` | New identity-only row from a name-only provider row |
| `gasly` | `pierre-gasly` | `Pierre Gasly` | New identity-only row |
| `hadjar` | `isack-hadjar` | `Isack Hadjar` | New identity-only row |
| `hamilton` | `lewis-hamilton` | `Lewis Hamilton` | Mapping only; identity already existed |
| `hulkenberg` | `nico-hulkenberg` | `Nico Hülkenberg` | New identity-only row |
| `jak_crawford` | `jak-crawford` | `Jak Crawford` | New identity-only row from a name-only provider row |
| `lawson` | `liam-lawson` | `Liam Lawson` | New identity-only row |
| `leclerc` | `charles-leclerc` | `Charles Leclerc` | Mapping only; identity already existed |
| `leonardo_fornaroli` | `leonardo-fornaroli` | `Leonardo Fornaroli` | New identity-only row from a name-only provider row |
| `luke_browning` | `luke-browning` | `Luke Browning` | New identity-only row from a name-only provider row |
| `max_verstappen` | `max-verstappen` | `Max Verstappen` | Mapping only; identity already existed; `permanentNumber` removed |
| `norris` | `lando-norris` | `Lando Norris` | Pre-existing mapping (§8.4), unchanged; `permanentNumber` removed from the identity |
| `ocon` | `esteban-ocon` | `Esteban Ocon` | New identity-only row |
| `paul_aron` | `paul-aron` | `Paul Aron` | New identity-only row from a name-only provider row |
| `perez` | `sergio-perez` | `Sergio Pérez` | New identity-only row |
| `piastri` | `oscar-piastri` | `Oscar Piastri` | Mapping only; identity already existed |
| `russell` | `george-russell` | `George Russell` | Mapping only; identity already existed |
| `ryo_hirakawa` | `ryo-hirakawa` | `Ryo Hirakawa` | New identity-only row from a name-only provider row |
| `sainz` | `carlos-sainz` | `Carlos Sainz` | New identity-only row |
| `stroll` | `lance-stroll` | `Lance Stroll` | New identity-only row |
| `tsunoda` | `yuki-tsunoda` | `Yuki Tsunoda` | New identity-only row |

All 33 identities are in `content/registries/drivers.mock.json`. All 32
provider values are mapped in `content/seasons/2026/provider-mappings.development.json`
and recorded in `content/seasons/2026/provider-evidence.development.json`, and
every new record cites this section and the response hash. A test reconstructs
this table from those files and fails if either side drifts.

**What this does and does not cover.** Curated driver identity coverage of the
observed 2026 list is **32 of 32**, and no Jolpica identity is acknowledged as
unmapped. The driver registry holds **33** identities and there are **32**
Jolpica driver mappings. The season-2026 dataset holds **93 exact mappings**
and **96 approved evidence identities**, with **three acknowledgements**
remaining, all OpenF1: `driver_number` `12`, `Cadillac` and `Racing Bulls`,
each with the reason `no-approved-provider-mapping`. Constructors stay at 11 /
11, and events and circuits at 23 / 23.

Complete participant identity coverage is not a participants resource: no
drivers, constructors or participants port exists, no participant span is
derived, and no result or standing is available. The ADR 0026 implementation
prerequisites remain open. The two existing Jolpica ports, `season-calendar`
and `season-circuits`, remain dormant. `PROVIDER_MODE` still admits exactly
`mock` and `none`, and **no live provider mode has been enabled**. Nothing was
deployed and no cutover occurred. The new identities feed the mock provider,
so a later deploy of a `master` that contains them changes the staging mock
snapshot and is cutover-sensitive.

> **Note 2026-09-24 (v1.11).** The port statements in this section and in
> §8.9 describe the repository when each dataset merged. A dormant,
> fixture-tested `season-participants` port now resolves these driver and
...
> specified but not unlocked, and the reconciliation path is specified but not
> built.

> **The participants resource (v1.11, 2026-09-24).** The `season-participants`
> port makes **two sequential requests** for one resource:
> `GET /ergast/f1/{season}/drivers/?limit=100`, then
> `GET /ergast/f1/{season}/constructors/?limit=100`, both through the
> hardened boundary and the Jolpica limiter. The execution fails fast: the
> constructors request is never begun after an unusable drivers response, and
> both identity lists come back or none does. Under
> [ADR 0023 A1](../adr/0023-multi-source-provider-coordination.md#amendment-a1---ordered-attempts-and-interrupted-executions)
> every request that reached transport is reported once, in order. A
> cancellation or limiter refusal between the two requests is a closed
> `interrupted` outcome carrying only the drivers attempt. A refusal before
> the first request is `not-attempted`. Only `driverId`, `constructorId` and
> strict pagination are decoded. Names and facts come from the curated
> registries. The payload holds the identity inventory, one
> `ConstructorSeasonEntry` per constructor and an empty `driverEntries`,
> because spans are season-assembly work (ADR 0026 D3, D11). **The evidence
> is fixture-only**: the tests replay synthetic envelopes built from committed
...
resource continues to be checked, and where a resource is still **unreconciled**
...
> **The coordination mechanism was implemented in Phase 9B-4**
> ([ADR 0023](../adr/0023-multi-source-provider-coordination.md)), closing G4.
> Each source is an independent port asked for one resource at a time; the
> coordinator above them is the only component that knows both, owns role and
> capability policy, and selects by declared role alone. **Neither adapter
> exists yet**, so nothing consumes the seam: it is dormant, and no provider
> request is possible. Removing either source still requires no public v1
> contract change — that property is now structural rather than aspirational,
> because no adapter is able to depend on the other's existence.
...
> **Ownership note 2026-09-23 (v1.8).** Under
> [ADR 0026](../adr/0026-season-participation-semantics-and-derivation.md) the
> participants identity refresh is `/{season}/drivers/?limit=100` and
> `/{season}/constructors/?limit=100`. Participation is derived from the race
> classifications the session reconciliation path (§10.4, §11.1) already
> fetches, so no additional result request exists. The "participants and
> circuits (3 calls)" line above is left as modelled. Before runtime wiring it
> must be reconciled so that each request is counted at its actual owning
> resource. None of these figures has been measured.
...
**Implemented in Phase 9B-2 (2026-08-23).** The limiter below now exists as a
Durable Object with one identity per real source, so pacing is one global
...
| G-c | ~~`fetchSeasonSource(season, jobs)` is a single coarse call and **cannot express two sources with different roles**; partial-failure semantics are undefined.~~ **Closed in Phase 9B-4 (2026-08-26)** ([ADR 0023](../adr/0023-multi-source-provider-coordination.md)). Per-resource, per-source typed requests and outcomes; source role and capability owned above the adapters; complete partial-success and partial-failure semantics; deterministic role-based selection; exact once-per-request accounting. **Dormant: no adapter consumes it.** This closes the **mechanism**; real reconciliation additionally needs both adapters and **G9**. **Superseded 2026-09-21** (v1.6, Implementation Plan §14.0.16): a **dormant, fixture-tested Jolpica `season-calendar` port** now implements this seam's port contract, but is not registered with the seam, is absent from the runtime composition and the Worker entry point's import closure, is not selectable through `PROVIDER_MODE` and is invoked by no production coordinator, so **no production path drives the seam**. The complete adapters, real reconciliation and **G9** remain open. |
| G-d | The scheduler is not event-aware (§10.5). |
| G-e | ~~No curated identifier mapping registry exists, and §8.5 proves one is mandatory.~~ **Closed in Phase 9B-3 (2026-08-25)** ([ADR 0022](../adr/0022-curated-provider-identifier-mappings.md)). A season-qualified curated registry keyed on source, entity kind, exact provider field and exact provider value, matched by exact typed equality with no normalisation, slug minting or similarity matching. Structural **and** semantic validation (composite-key uniqueness, target existence, evidence coverage) run in `npm run validate:content`; one invalid record blocks the whole resolver. Unknown entities fail closed with a bounded operational signal. **Dormant: no adapter consumes it**, and coverage is limited to identifiers already recorded in §8. This closes the **mechanism** only; dataset coverage is tracked separately as **G-l**. **Superseded 2026-09-21** (v1.6, Implementation Plan §14.0.16): a **dormant, fixture-tested Jolpica `season-calendar` port** now resolves identities through this registry, but **only when exercised directly by tests** - it is absent from the runtime composition and the Worker entry point's import closure, is not selectable through `PROVIDER_MODE`, and is invoked by no production coordinator or event-aware scheduler, so **no deployed or application path consumes the registry**. **G-l** is unaffected. |
| G-f | ~~No outbound-request hardening helper exists.~~ **Closed in Phase 9B-2 (2026-08-23).** One hardened boundary pins both origins and path prefixes, is GET-only, rejects every 3xx, applies a 10-second whole-operation timeout and a 2 MiB streamed body cap, accepts only JSON media types, forwards no caller header or credential, and sends Jolpica's mandatory identifying `User-Agent` as a reviewed constant. |
...
**The gate would otherwise be circular.** E5a, E5b and E6 all concern things
Phase 9B *builds*: the `sourceUpdatedAt` resolution, the settling design, the
bound-or-skip guard, the rate limiter. Requiring them to be *implemented* before
Phase 9B may begin would mean Phase 9B work must be finished before Phase 9B can
...
The distinction matters most for E6. Entry needs the live-window rule specified;
**release** needs it demonstrably enforced, because that is the point at which a
mistake would put a real request inside OpenF1's paid window.
...
| G3 | **No production cron** | `wrangler.toml` declares `crons` only under `[env.staging.triggers]`. §10.3 needs fine-grained invocation in production (Q1). **Prerequisite, recorded 2026-09-03: production operational runbooks.** A production cron makes the Worker act on its own against live data, so the procedures an operator needs when it misbehaves - triggering and interpreting a manual synchronization, reading publication and quota state, rolling back, purging the cache, and the escalation path for each - must exist **before** the trigger is declared, not after the first incident. [`../operations/GridView_Staging_Edge_Runbook.md`](../operations/GridView_Staging_Edge_Runbook.md) is staging-scoped and does not satisfy this. The prerequisite is deliberately recorded here rather than in a release checklist, because G3 is the gap that would otherwise be closed by a two-line configuration change. |
| G4 | ~~**The provider interface cannot express two sources**~~ **Closed in Phase 9B-4 (2026-08-26)** | `fetchSeasonSource(season, jobs)` demanded a whole season from one call and defined no partial-success or per-job failure result. A typed multi-source coordination seam now supersedes it ([ADR 0023](../adr/0023-multi-source-provider-coordination.md)): independent per-source resource ports, a closed union of per-resource requests, source role and capability policy above the adapters, a closed outcome taxonomy separating not-attempted from attempted from coordination failures, deterministic selection by declared role, exact once-per-transport-request accounting, bounded cancellation and concurrency, and a guarded at-most-once bridge to the unchanged publisher. **The mechanism is closed; the dormancy is not an oversight.** No adapter consumes it, so no reconciliation happens and none is claimed. The deep normalized-contract validation gate recorded beside it (ADR 0023 D14) was closed as a mechanism by **Phase 9B-5** ([ADR 0024](../adr/0024-deep-normalized-contract-validation.md)); adapter registration still depends on per-source normalization evidence. |
| G5 | **No event-window awareness** | `scheduler.ts` intervals are constants. §10.3 and §10.4 need offsets relative to session end. |
| G6 | ~~**Untyped call counting**~~ **Implemented (Phase 9B-1, 2026-08-23)** | The cast to `{ callCount?: unknown }` is gone. `FormulaOneProvider` requires `readonly sourceId: ProviderSourceId`, `readonly quotaPolicy` and `requestMetrics(): ProviderRequestMetrics`, so an adapter without telemetry does not compile and no fallback to `0` exists. `SyncResult.providerCallCount` keeps its lifetime-total meaning for the existing internal admin responses, and `SyncResult.providerRequests` adds operation-scoped and lifetime counts by canonical source and by job category, separating successful, failed and rate-limited attempts. |
| G7 | ~~**No HTTP hardening helper and no rate limiter**~~ **Implemented (Phase 9B-2, 2026-08-23)** | One hardened outbound boundary (`src/providers/http/provider-http-client.ts`) supplies every §23.3 control: pinned HTTPS origins and documented path prefixes, GET only, `redirect: "manual"` with every 3xx rejected, a 10-second whole-operation timeout, JSON-only content types, a 2 MiB decoded cap enforced by both `Content-Length` and bounded streaming, no automatic retry, no caller header or credential forwarded, Jolpica's identifying `User-Agent` as a reviewed constant, and logs that never carry a body, URL, query string, header or raw exception. The **per-provider rate limiter** is a Durable Object with one identity per real source, performing exact sliding-window reservations across every published window atomically ([ADR 0021](../adr/0021-hardened-provider-boundary-and-durable-object-rate-limiter.md)). **No adapter exists and no provider request is sent.** **Superseded 2026-09-21** (v1.6, Implementation Plan §14.0.16): a **dormant, fixture-tested Jolpica `season-calendar` port** now reserves through the limiter and uses this boundary's transport type, but **only when exercised directly by tests against fixtures**; it is absent from the runtime composition and the Worker entry point's import closure, is not selectable through `PROVIDER_MODE` and is invoked by no production coordinator, so **no provider request is sent** and the complete adapter remains unimplemented. |
| G8 | ~~**No provider-ID mapping registry**~~ **Closed in Phase 9B-3 (2026-08-25)** | Backend Scheme §8.1 required one and §8.5 proved it mandatory: 4 of 11 constructor names differ between sources. A curated, season-qualified, exactly-matched registry now exists with a fail-closed immutable resolver ([ADR 0022](../adr/0022-curated-provider-identifier-mappings.md)). It is **dormant** until an adapter exists, and it seeds only identifiers already recorded in §8 — `Cadillac` and `Racing Bulls` stay unmapped because no canonical GridView constructor identity exists for them, and no OpenF1 `circuit_key` value is recorded anywhere. **This closes the mechanism, not the dataset**: coverage remains incomplete and is tracked as **G-l**. **Superseded 2026-09-21** (v1.6, Implementation Plan §14.0.16): a **dormant, fixture-tested Jolpica `season-calendar` port** now resolves through this registry **when exercised directly by tests**, while staying outside the runtime composition and the Worker entry point's import closure, unselectable through `PROVIDER_MODE` and invoked by no production coordinator, so **no deployed or application path resolves through it**. The registry's dormancy, the dataset gap and **G-l** are unchanged. |
...
| G10 | ~~**No locally-modelled quota state**~~ **Implemented (Phase 9B-1, 2026-08-23)** | `QuotaState` is keyed by canonical source and holds an extensible collection of the windows that source publishes, with GridView-local usage and remaining counts, window start/reset, a bounded burst-saturation streak, last provider success and failure, `Retry-After`, usage by job category and a derived warning level. Persisted under `quota:provider:<sourceId>` in both the memory and KV implementations, with a narrow mock-only fallback to the legacy `quota:provider` record whose daily and per-minute figures are discarded rather than reinterpreted. |

**None of G1-G10 was implemented, scaffolded or stubbed in the Phase 9A pass.**
No provider client, provider DTO, authentication code, secret name, environment
variable, Worker route, provider-specific mapping, cron trigger, network test or
production configuration was added or changed by it.

**Phase 9B-1 (2026-08-23) subsequently implemented G6 and G10 only.** G1-G5 and
G7-G9 remain open, and no provider client, provider DTO, outbound request
helper, credential, cron trigger, network test, Cloudflare resource or
production configuration was added by that pass either. Naming `jolpica` and
`openf1` as canonical internal source identifiers, and recording their published
rate-limit windows, is quota modelling: it creates no adapter and does not
unlock the OpenF1 path, which stays fail-closed under ADR 0020 §5.

**Phase 9B-2 subsequently closed G7, Phase 9B-3 closed G8 and Phase 9B-4 closed
G4** — each as a mechanism, each dormant. **Phase 9B-5 (2026-09-02) closed
the remaining ADR 0023 D14 activation gate, deep normalized-contract
validation**, also as a dormant mechanism, together with the deferred F3, F4
and F5 referential findings
([ADR 0024](../adr/0024-deep-normalized-contract-validation.md)). Registering a
real adapter is still gated on that adapter's own per-source normalization
being correct, which needs recorded evidence rather than more validation code.
**G1, G3, G5, G9 and G-l remain open.** None of those passes added a provider adapter, a provider DTO, a
credential, a cron trigger, a network test, a Cloudflare resource or a
production configuration, and none unlocked the OpenF1 path, which still has no
recorded maximum-session-duration bound.
